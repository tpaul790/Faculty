...
void addCarry(int* resultChunk, const int& resultChunkSize, int& carry) {
    for (int i = 0; i < resultChunkSize && carry > 0; i++) {
        const int sum = resultChunk[i] + carry;
        resultChunk[i] = sum % 10;
        carry = sum / 10;
    }
}
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        //nu astept sa primesc carry
        int localCarryOut = addChunks(digitChunk1, digitChunk2, resultChunk, logicChunkSize, 0);
        ...
        if (carryIn > 0) {
            addCarry(resultChunk, logicChunkSize, carryIn);
        }

        int finalCarryOut = localCarryOut + carryIn;

        if (rank < workersCount) {
            MPI_Send(&finalCarryOut, 1, MPI_INT, rank + 1, CARRY_TAG, MPI_COMM_WORLD);
        } else {
            MPI_Send(&finalCarryOut, 1, MPI_INT, 0, CARRY_TAG, MPI_COMM_WORLD);
        }
